...
const $sensorLastInspectionDate = document.querySelector(
  ".sensor-last-inspection-date"
);
...
const fillSensorTable = (sensorData) => {
  ...
  $sensorLastInspectionDate.textContent =
    sensorData.inspection.lastInspectionDate;
...
const fillWeatherForecastTable = (weatherForecastData) => {
  ...
  $weatherIcon.setAttribute(
    "src",
    weatherForecastData.current.weather_icons[0]
  );
  $weatherIcon.setAttribute(
    "alt",
    weatherForecastData.current.weather_descriptions[0]
  );
  ...
  $weatherIsDay.textContent =
    weatherForecastData.current.is_day === "yes" ? "Jour" : "Nuit";
};
